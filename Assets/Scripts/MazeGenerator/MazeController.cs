...
public class MazeController : MonoBehaviour {
    ...
    private void GenerateWalls(MazeCell cell, GameObject cellGameObject) {
        ...
        if(cell.WallRight && cell.Column + 1 < Columns) {
            wall = Instantiate(Wall, new Vector3(cell.X + LenghtOfCellSide / 2, 0, cell.Y) + Wall.transform.position, Quaternion.Euler(0, 90, 0)) as GameObject;// right
            wall.name = "WallRight";
            wall.transform.parent = cellGameObject.transform;
        }
        if(cell.WallFront && cell.Row + 1 < Rows) {
            wall = Instantiate(Wall, new Vector3(cell.X, 0, cell.Y + LenghtOfCellSide / 2) + Wall.transform.position, Quaternion.Euler(0, 0, 0)) as GameObject;// front
            wall.name = "WallFront";
            wall.transform.parent = cellGameObject.transform;
        }
        if(cell.WallLeft && cell.Column > 0) {
            wall = Instantiate(Wall, new Vector3(cell.X - LenghtOfCellSide / 2, 0, cell.Y) + Wall.transform.position, Quaternion.Euler(0, 270, 0)) as GameObject;// left
            wall.name = "WallLeft";
            wall.transform.parent = cellGameObject.transform;
        }
        if(cell.WallBack && cell.Row > 0) {
            wall = Instantiate(Wall, new Vector3(cell.X, 0, cell.Y - LenghtOfCellSide / 2) + Wall.transform.position, Quaternion.Euler(0, 180, 0)) as GameObject;// back
            wall.name = "WallBack";
            wall.transform.parent = cellGameObject.transform;
        }
